...
function CTAButtons({ slug }: { slug: string }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 mb-6">
      <Link href={`/fr/contact?service=${slug}`} className="bg-emerald-700 text-white font-bold px-8 py-4 rounded-2xl hover:bg-emerald-800 text-center shadow-lg">
        Démo Sans Engagement
      </Link>
      {showPhone ? (
        <a href={`tel:${CONTACT.phone}`} className="border-2 border-emerald-700 text-emerald-700 font-bold px-8 py-4 rounded-2xl hover:bg-emerald-700 hover:text-white transition-all text-center">
          {CONTACT.phoneDisplay}
        </a>
      ) : (
        <a href={CONTACT.whatsapp} target="_blank" rel="noopener noreferrer" className="border-2 border-emerald-700 text-emerald-700 font-bold px-8 py-4 rounded-2xl hover:bg-emerald-700 hover:text-white transition-all text-center">
          💬 WhatsApp 24/7
        </a>
      )}
    </div>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        <div className="w-full lg:w-1/2 animate-slide-left">
          ...
          <h1 className="text-4xl lg:text-5xl font-black mb-6 leading-tight text-slate-900">
            Support Client<br/>
            <span className="bg-gradient-to-r from-emerald-600 to-teal-700 bg-clip-text text-transparent">Qui Ressemble à Vous</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3">
            {['Multicanal', 'Tickets organisés', 'Réponses < 2h', 'Équipe dédiée'].map((b, i) => (
              <span key={b} className={`flex items-center gap-2 bg-white text-slate-700 text-sm font-medium px-4 py-2 rounded-full shadow-md animate-fade-in-up animate-delay-${(i+1)*100}`}><CheckIcon className="w-5 h-5 text-emerald-600" /> {b}</span>
            ))}
          </div>
        </div>
        <div className="w-full lg:w-1/2 animate-slide-right">
          <div className="relative">
            ...
            <img src="/smart-hotline-last/images/support-tech.jpg" alt="Support client" className="relative rounded-3xl shadow-2xl w-full object-cover hero-image-zoom" style={{maxHeight:'550px', objectFit:'cover'}}/>
