...
function CTAButtons({ slug }: { slug: string }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 mb-6">
      <Link href={`/fr/contact?service=${slug}`} className="bg-purple-600 text-white font-bold px-8 py-4 rounded-2xl hover:bg-purple-700 text-center shadow-lg">
...
{/* SECTION 1: LIGHT - Hero */}
  <section className="bg-gradient-to-br from-slate-50 via-white to-purple-50 text-slate-900 py-16 lg:py-24 overflow-hidden">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        <div className="w-full lg:w-1/2 animate-slide-left">
          <span className="inline-flex items-center gap-2 bg-purple-100 text-purple-700 text-sm font-semibold px-4 py-2 rounded-full mb-6 animate-slow-float">
            <CRMIcon className="w-5 h-5" /> CRM & Listes
          </span>
          <h1 className="text-4xl lg:text-5xl font-black mb-6 leading-tight text-slate-900">
            CRM Intégré &<br/>
            <span className="bg-gradient-to-r from-purple-600 to-indigo-700 bg-clip-text text-transparent">Listes de Prospects</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3">
            {['SuiteCRM inclus', 'Listes B2B/B2C', 'Mautic intégré', 'Rapports auto'].map((b, i) => (
              <span key={b} className={`flex items-center gap-2 bg-white text-slate-700 text-sm font-medium px-4 py-2 rounded-full shadow-md animate-fade-in-up animate-delay-${(i+1)*100}`}><CheckIcon className="w-5 h-5 text-purple-600" /> {b}</span>
            ))}
          </div>
        </div>
        <div className="w-full lg:w-1/2 animate-slide-right">
          <div className="relative">
            <div className="absolute -inset-4 bg-gradient-to-r from-purple-400 to-indigo-500 rounded-3xl blur-2xl opacity-20"></div>
            <img src="/smart-hotline-last/images/crm-interface.jpg" alt="CRM dashboard" className="relative rounded-3xl shadow-2xl w-full object-cover hero-image-zoom" style={{maxHeight:'550px', objectFit:'cover'}}/>
...
<section className="bg-gradient-to-br from-slate-900 via-purple-950 to-purple-900 text-white py-20 lg:py-28 overflow-hidden relative">
