      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16 gap-6">
          <Link href={lang === 'fr' ? '/fr' : '/en'}
            className="flex items-center gap-2 font-extrabold text-lg text-blue-800 flex-shrink-0">
            ...
            {nav.map((item) =>
              item.children ? (
                <div key={item.label} className="relative"
                  ...
                  <button className="flex items-center gap-1 px-3 py-2 text-slate-700 hover:text-blue-700 font-medium text-sm rounded-lg hover:bg-blue-50 whitespace-nowrap transition-colors">
                    {item.label}
          ...
          <div className="hidden lg:flex items-center gap-3 ml-auto flex-shrink-0">
            {showPhone && (
              <a href={"tel:" + CONTACT.phone}
                className="flex items-center gap-1.5 text-sm text-slate-700 hover:text-blue-700 font-medium">
            ...
            {!showPhone && !loading && (
              <a href={CONTACT.whatsapp} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-sm text-slate-700 hover:text-green-600 font-medium">
            ...
            <Link href={contactHref}
              className="bg-blue-700 text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-blue-800 transition-colors shadow-sm">
              {ctaLabel}
            </Link>
          </div>

          <button className="lg:hidden ml-auto p-2 rounded-lg hover:bg-slate-100"
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-slate-100 px-4 py-4 shadow-lg">
          ...
          <div className="mt-4 space-y-2">
            {showPhone && (
              <a href={"tel:" + CONTACT.phone}
                className="flex items-center justify-center gap-2 py-3 border-2 border-slate-200 text-slate-700 rounded-xl font-semibold text-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                </svg>
                {CONTACT.phoneDisplay}
            ...
            <Link href={contactHref}
              onClick={() => setMobileOpen(false)}
              className="block text-center bg-blue-700 text-white py-3 rounded-xl font-semibold text-sm">
              {ctaLabel}
            </Link>
          </div>
        </div>
